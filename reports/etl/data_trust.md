...
| Dataset | Score | Nota |
| --- | ---: | :---: |
| `data/raw` (crudo) | 91.38 | C |
| `data/processed` (limpio) | 100.00 | A |

## Antes de limpiar

**Data Trust Score: 91.38 / 100 (nota C)** — 70 de 79 comprobaciones sin una sola fila mala.
...
| `consistency` | 84.82 | 98.22 % | 71.43 % | 2 / 7 |
| `integrity` | 100.00 | 100.00 % | 100.00 % | 0 / 9 |

| Tabla | Dimension | Comprobacion | Filas KO | Total | % OK |
| --- | --- | --- | ---: | ---: | ---: |
| `baskets` | consistency | `total_amount` cuadra con la suma de sus lineas | 73.007 | 600.174 | 87.836 % |
| `products` | validity | `category` sin mayusculas ni espacios inconsistentes | 20 | 496 | 95.968 % |
| `basket_items` | uniqueness | Una sola linea por cesta y producto | 64.512 | 4.357.007 | 98.519 % |
| `basket_items` | uniqueness | Sin filas duplicadas exactas | 63.994 | 4.357.007 | 98.531 % |
| `products` | completeness | `brand` informada | 5 | 496 | 98.992 % |
| `customers` | completeness | `city` informada | 154 | 20.000 | 99.230 % |
| `basket_items` | validity | `quantity` mayor que 0 | 17.253 | 4.357.007 | 99.604 % |
| `customers` | consistency | `signup_date` anterior o igual a la primera compra | 58 | 20.000 | 99.710 % |
| `session_events` | uniqueness | Sin filas duplicadas exactas | 85 | 1.106.225 | 99.992 % |
| `basket_items` | integrity | `basket_id` existe en `basket_id` | 0 | 4.357.007 | 100.000 % |
| `basket_items` | completeness | `basket_id` informada | 0 | 4.357.007 | 100.000 % |
| `basket_items` | integrity | `product_id` existe en `product_id` | 0 | 4.357.007 | 100.000 % |
| `basket_items` | completeness | `product_id` informada | 0 | 4.357.007 | 100.000 % |
| `basket_items` | consistency | La promocion aplicada estaba vigente el dia de la cesta | 0 | 212.009 | 100.000 % |
| `basket_items` | integrity | `promotion_id` existe en `promotion_id` | 0 | 212.009 | 100.000 % |
| `basket_items` | completeness | `quantity` informada | 0 | 4.357.007 | 100.000 % |
| `basket_items` | completeness | `unit_price_paid` informada | 0 | 4.357.007 | 100.000 % |
| `basket_items` | validity | `unit_price_paid` mayor que 0 | 0 | 4.357.007 | 100.000 % |
...
| `session_events` | consistency | El evento no es anterior al inicio de su sesion | 0 | 1.106.225 | 100.000 % |
| `session_events` | completeness | `event_timestamp` informada | 0 | 1.106.225 | 100.000 % |
| `session_events` | validity | `event_type` en view/add_to_cart | 0 | 1.106.225 | 100.000 % |
| `session_events` | completeness | `event_type` informada | 0 | 1.106.225 | 100.000 % |
| `session_events` | integrity | `product_id` existe en `product_id` | 0 | 1.106.225 | 100.000 % |
| `session_events` | completeness | `product_id` informada | 0 | 1.106.225 | 100.000 % |
| `session_events` | integrity | `session_id` existe en `session_id` | 0 | 1.106.225 | 100.000 % |
| `session_events` | completeness | `session_id` informada | 0 | 1.106.225 | 100.000 % |
...
| `sessions` | integrity | `customer_id` existe en `customer_id` | 0 | 115.350 | 100.000 % |
...
| Tabla | Dimension | Comprobacion | Filas KO | Total | % OK |
| --- | --- | --- | ---: | ---: | ---: |
| `basket_items` | integrity | `basket_id` existe en `basket_id` | 0 | 4.292.495 | 100.000 % |
| `basket_items` | completeness | `basket_id` informada | 0 | 4.292.495 | 100.000 % |
| `basket_items` | uniqueness | Una sola linea por cesta y producto | 0 | 4.292.495 | 100.000 % |
| `basket_items` | uniqueness | Sin filas duplicadas exactas | 0 | 4.292.495 | 100.000 % |
| `basket_items` | integrity | `product_id` existe en `product_id` | 0 | 4.292.495 | 100.000 % |
| `basket_items` | completeness | `product_id` informada | 0 | 4.292.495 | 100.000 % |
| `basket_items` | consistency | La promocion aplicada estaba vigente el dia de la cesta | 0 | 208.961 | 100.000 % |
| `basket_items` | integrity | `promotion_id` existe en `promotion_id` | 0 | 208.961 | 100.000 % |
| `basket_items` | completeness | `quantity` informada | 0 | 4.292.495 | 100.000 % |
| `basket_items` | validity | `quantity` mayor que 0 | 0 | 4.292.495 | 100.000 % |
| `basket_items` | completeness | `unit_price_paid` informada | 0 | 4.292.495 | 100.000 % |
| `basket_items` | validity | `unit_price_paid` mayor que 0 | 0 | 4.292.495 | 100.000 % |
| `baskets` | completeness | `basket_date` informada | 0 | 600.174 | 100.000 % |
| `baskets` | completeness | `basket_id` informada | 0 | 600.174 | 100.000 % |
| `baskets` | uniqueness | `basket_id` sin repetir | 0 | 600.174 | 100.000 % |
| `baskets` | validity | `channel` en app/web/store | 0 | 600.174 | 100.000 % |
...
| `promotions` | uniqueness | `promotion_id` sin repetir | 0 | 300 | 100.000 % |
| `promotions` | completeness | `start_date` informada | 0 | 300 | 100.000 % |
| `session_events` | consistency | El evento no es anterior al inicio de su sesion | 0 | 1.106.140 | 100.000 % |
| `session_events` | completeness | `event_timestamp` informada | 0 | 1.106.140 | 100.000 % |
| `session_events` | validity | `event_type` en view/add_to_cart | 0 | 1.106.140 | 100.000 % |
| `session_events` | completeness | `event_type` informada | 0 | 1.106.140 | 100.000 % |
| `session_events` | uniqueness | Sin filas duplicadas exactas | 0 | 1.106.140 | 100.000 % |
| `session_events` | integrity | `product_id` existe en `product_id` | 0 | 1.106.140 | 100.000 % |
| `session_events` | completeness | `product_id` informada | 0 | 1.106.140 | 100.000 % |
| `session_events` | integrity | `session_id` existe en `session_id` | 0 | 1.106.140 | 100.000 % |
| `session_events` | completeness | `session_id` informada | 0 | 1.106.140 | 100.000 % |
| `sessions` | integrity | `basket_id` existe en `basket_id` | 0 | 52.500 | 100.000 % |
| `sessions` | consistency | `converted` coincide con tener `basket_id` | 0 | 150.000 | 100.000 % |
| `sessions` | completeness | `converted` informada | 0 | 150.000 | 100.000 % |
| `sessions` | integrity | `customer_id` existe en `customer_id` | 0 | 115.350 | 100.000 % |
